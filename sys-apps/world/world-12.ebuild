...
IUSE="amd anaconda archive ath9k +bfq bmq broadcom +browser btrfs bzip2 +chrome +composite +composite-gui debug dev discord +efi exfat ext2 f2fs +fat firefox flash fluxbox fslint ftp +fvwm gentoo-patches gentoo-sources +gnomekeyring-admin gzip hygon +index intel irc ivybridge kate kyber latex logisim lz4 lzma +lzo mail-client mips-emulate +network-tray odt openbox openrc overlay parted pdf +pf-sources +processviewer remote-desktop screenshot secure spreadsheet steam sublime sublimelinter +sudo system-flake8 system-pylint +systemd terminal +terminal-fast test +text-editor text-editor-color-highlight text-editor-js-complete text-editor-js-lint text-editor-py-complete text-editor-py-lint threads-4 threads-16 threads-512 +uefi viewfont xeon +xfs xz yandex-disk zhaoxin"
S="${WORKDIR}"
...
RDEPEND="
	...
	fvwm? ( x11-wm/fvwm lxqt-base/lxqt-meta[minimal] )
